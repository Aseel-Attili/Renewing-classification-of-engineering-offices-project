...
def page():
    ...
    counter = 0
    app = ctk.CTk()
    app.title('Full-time engineers Information')
    ...
    def add_to_treeview():
        ...
        for item_id in item_ids:
            ...
            if (rg == registration_number):
                messagebox.showerror(title='Registered Full-Time Engineer', message='Full-Time Engineer already exists!')
                return
    ...
    def insert_button_function():
        ...
        cursor.execute("""select * from Engineers where Registration_Number = ? and Job_title = 'full-time'""",(reg,))
    ...
    def nextPage():
        ...
        connection = sqlite3.connect(File)
        cursor = connection.cursor()
        cursor.execute("""select * from Engineers where Classification_Number = ? and Job_title = 'full-time'""",(classification_num,))
        find = cursor.fetchall()
        if counter < len(find):
            messagebox.showwarning(title='Incomplete entry', message='You need to enter all of your office full_time engineers ')
            return
        
        app.destroy()
        office_official_six.page()
